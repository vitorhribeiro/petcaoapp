...
export default function AuthPage() {
  ...
  const handleRegContinue = () => {
    ...
    if (!isValidPhone(regPhone)) { setError('Telefone inválido.'); return; }
  ...
  const handleRegisterFinish = async () => {
    ...
    setLoading(false);
    if (!res.success) { if (res.error === 'PRE_REGISTERED') setRegStep(1); setError(res.error || 'Erro ao criar conta.'); }
  };
           ...
           {!isLogin && (
             <div className="space-y-5 py-2 animate-in fade-in slide-in-from-right-4 duration-500">
               <div className="space-y-0.5">
                 ...
                 <p className="text-muted-foreground dark:text-white/80 text-xs">{regStep === 1 ? 'Preencha seus dados para começar.' : 'Agora, cadastre seu pet.'}</p>
               </div>
               <div className="flex gap-2 mb-1"><div className={cn("h-1 flex-1 rounded-full transition-all", regStep >= 1 ? "bg-primary" : "bg-muted")} /><div className={cn("h-1 flex-1 rounded-full transition-all", regStep >= 2 ? "bg-primary" : "bg-muted")} /></div>

               {regStep === 1 ? (
                 <div className="space-y-3">
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     <div className="space-y-1"><Label className="text-xs dark:text-white/70">Nome Completo</Label><div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground dark:text-white/40" /><Input value={regName} onChange={e => setRegName(e.target.value)} placeholder="Seu nome" className="pl-11 h-11 rounded-xl text-sm dark:bg-white/5 dark:border-white/10 dark:placeholder:text-white/30 dark:text-white" /></div></div>
                     <div className="space-y-1"><Label className="text-xs dark:text-white/70">Telefone</Label><PhoneInput value={regPhone} onChange={setRegPhone} /></div>
                   </div>
                   ...
                   {error && regStep === 1 && <p className="text-xs text-destructive font-medium">{error}</p>}
                 ...
                 <div className="space-y-4">
                   <button onClick={() => setRegStep(1)} className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="w-3.5 h-3.5" /> Voltar</button>
                   <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                     {regPets.map((pet, idx) => (
                       <div key={idx} className="p-4 rounded-2xl border border-border/40 bg-muted/20 relative space-y-3">
                         <div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2"><PawPrint className="w-3.5 h-3.5 text-primary" /> Pet {idx + 1}</span>{regPets.length > 1 && (<button onClick={() => setRegPets(prev => prev.filter((_, i) => i !== idx))} className="text-destructive p-1 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-4 h-4" /></button>)}</div>
                   ...
                   {error && regStep === 2 && <p className="text-xs text-destructive font-medium text-center">{error}</p>}
                   <Button onClick={handleRegisterFinish} disabled={loading || !lgpdAccepted} className="w-full h-14 rounded-2xl font-bold shadow-lg shadow-primary/20 dark:text-white">{loading ? 'Criando...' : 'Finalizar Cadastro'}</Button>
                 </div>
               )}
             </div>
           )}
